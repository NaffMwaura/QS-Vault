...
const BlueprintViewport: React.FC<BlueprintViewportProps> = ({
  ...
  return (
    <section className="theme-page flex-1 relative overflow-hidden flex flex-col items-center justify-center transition-colors duration-500">
      
      {/* 1. VIEWPORT HUD (Floating Controls) */}
      <div className="theme-panel absolute top-4 left-4 sm:top-6 sm:left-6 flex flex-col gap-2 z-40 p-1.5 rounded-2xl border shadow-2xl backdrop-blur-xl transition-colors">
        
        <button onClick={() => setScale(s => Math.min(s + 0.25, 5))} className="theme-icon p-3 transition-all active:scale-90 rounded-xl hover:text-amber-500 hover:bg-black/5" title="Increase Scale">
          <ZoomIn size={20}/>
        </button>
        
        <button onClick={() => setScale(s => Math.max(s - 0.25, 0.25))} className="theme-icon p-3 transition-all active:scale-90 rounded-xl hover:text-amber-500 hover:bg-black/5" title="Decrease Scale">
          <ZoomOut size={20}/>
        </button>

        <div className="h-px mx-2 my-1 theme-divider border-t" />
        ...
        <button 
          ...
          className={`p-3 rounded-xl transition-all shadow-xl active:scale-90 
            ${!isMeasuring ? 'bg-amber-500 text-black shadow-amber-500/20' : 'theme-icon hover:bg-black/5 hover:[color:var(--app-heading)]'} 
      ...
      <div className="absolute top-6 right-6 z-40 max-w-[min(22rem,calc(100%-2rem))] hidden md:block">
        <div className="theme-panel border-2 rounded-[1.75rem] p-4 sm:p-5 shadow-2xl backdrop-blur-xl transition-colors">
          <div className="flex items-center gap-3 mb-3">
            <Ruler size={16} className="text-amber-500" />
            <p className="theme-meta text-[9px] font-black uppercase tracking-[0.28em]">
              Active Context
            </p>
          </div>
          <p className="theme-heading text-sm font-semibold leading-snug">
            Upload the drawing, confirm the ratio, then capture measurements.
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            <span className="theme-card px-3 py-2 rounded-full border text-[9px] font-black uppercase tracking-[0.2em]">
      ...
      >
        {!pdfDoc ? (
          <div className="w-full h-full flex items-center justify-center">
            <div className="theme-panel p-8 sm:p-24 text-center backdrop-blur-md rounded-[3rem] border-2 flex flex-col items-center gap-8 m-4 max-w-2xl transition-colors">
              <div className="relative">
                <div className="absolute inset-0 bg-amber-500/10 rounded-full blur-3xl animate-pulse" />
                <div className="theme-card p-12 rounded-full border-2 shadow-inner relative transition-colors">
                  <FileSearch size={64} className="theme-icon opacity-60" />
                </div>
              </div>
              
              <div className="space-y-6">
                <div className="space-y-2">
                  <p className="theme-meta font-black uppercase tracking-[0.5em] text-[10px] leading-none italic">
                    Node Offline
                  </p>
                  <h3 className="theme-heading text-2xl font-black italic uppercase tracking-tighter">
                    Blueprint Required
                  </h3>
                  <p className="theme-meta text-sm max-w-lg mx-auto font-medium">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  ...
                  <button onClick={loadSampleBlueprint} className="theme-button-secondary px-10 py-5 border-2 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-3 active:scale-95">
      ...
      <div className="absolute bottom-6 left-6 z-40 pointer-events-none">
        <div className="theme-panel backdrop-blur-xl border px-6 py-4 rounded-3xl flex items-center gap-4 shadow-2xl transition-colors">
          ...
          <div className="text-left">
            <p className="theme-meta text-[8px] font-black uppercase tracking-[0.3em] leading-none mb-1.5">Engine Status</p>
            <div className="flex items-center gap-2">
               {isRendering && <Loader2 size={12} className="animate-spin text-amber-500" />}
               <p className={`text-[11px] font-black italic uppercase tracking-widest leading-none ${isRendering ? 'text-amber-500' : 'theme-heading'}`}>
        ...
        <div className="absolute bottom-6 right-6 z-40 pointer-events-none hidden sm:block">
          <div className="theme-panel backdrop-blur-xl border px-6 py-4 rounded-3xl flex items-center gap-4 shadow-2xl transition-colors">
            <div className="text-right">
              <p className="theme-meta text-[8px] font-black uppercase tracking-[0.3em] leading-none mb-1.5">Vault Ledger</p>
              <div className="flex items-center justify-end gap-2">
                <BadgeCheck size={12} className="text-emerald-500" />
                <span className="theme-heading text-[11px] font-black uppercase tracking-[0.2em]">
      ...
      <style>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: var(--app-border); border-radius: 20px; border: 2px solid transparent; background-clip: content-box; }
