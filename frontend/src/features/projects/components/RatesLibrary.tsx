...
const RatesLibrary: React.FC = () => {
  ...
  const handleUpdateRate = async () => {
    ...
    try {
      ...
      setRates((prev) =>
        prev.map((r) =>
          r.id === editingRate.id ? { ...r, rate: updatedRate } : r,
        ),
      );
      ...
      if (ratesTable) {
        await ratesTable.update(editingRate.id, {
          rate: updatedRate,
      ...
      // 3. Queue Cloud Sync
      if (syncEngine) {
        await syncEngine.queueChange("rates", editingRate.id, "UPDATE", {
          rate: updatedRate,
        });
      }
    ...
    <div className="theme-panel p-8 sm:p-12 transition-all duration-500 relative">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-10 mb-16 text-left">
        ...
        <div className="relative w-full md:w-96 group text-left">
          <Search
            className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-amber-500 transition-colors"
            size={20}
          />
          <input
            type="text"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {filteredRates.length > 0 ? (
              ...
              >
                ...
                <div className="theme-divider pt-8 border-t flex justify-between items-center">
                  <div className="text-left">
                    <p className="text-[8px] font-black uppercase text-zinc-500 mb-1 leading-none tracking-widest text-left">
                      Standard Rate
                    </p>
            ...
            <div className="col-span-full py-32 text-center border-2 border-dashed border-zinc-800 rounded-[4rem] opacity-20">
              ...
              <p className="font-black uppercase text-sm tracking-[0.5em]">
                No matching items in office database
              </p>
            </div>
          )}
        </div>
          ...
          <div className="theme-panel w-full max-w-xl p-10 sm:p-14 shadow-2xl transition-all duration-500 transform animate-in zoom-in-95">
            <div className="flex justify-between items-start mb-12">
              <div className="text-left space-y-2">
                <div className="flex items-center gap-3">
                  <span className="px-3 py-1 rounded bg-amber-500/10 border border-amber-500/20 text-amber-500 font-mono text-[10px] font-black">
                    {editingRate.code}
                  </span>
              ...
              <button
                ...
                className="theme-surface-inset theme-muted p-4 rounded-2xl hover:text-[var(--app-fg)] transition-colors"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="space-y-10">
              <div className="space-y-4 text-left">
                ...
                <p className="text-[9px] font-bold text-zinc-600 uppercase tracking-widest ml-4">
                  Rate is calculated per {editingRate.unit}.
                </p>
              </div>
            ...
            <div className="mt-12 pt-8 border-t border-zinc-800/40 flex items-center gap-3 opacity-40">
              ...
              <p className="text-[8px] font-black uppercase tracking-widest">
                Rate changes will affect future BoQ computations.
              </p>
            </div>
          </div>
        </div>
