...
import { Database, Globe, WifiOff, ClipboardList } from "lucide-react";
...
const StatCard: React.FC<{ item: StatItem; theme: "light" | "dark" }> = ({
  ...
  <div
    className={`p-8 sm:p-10 rounded-[2.5rem] sm:rounded-[3rem] border backdrop-blur-3xl flex justify-between items-center group transition-all duration-500 hover:scale-[1.02]
    ${
      theme === "dark"
  ...
  >
    <div className="text-left space-y-3">
      <p className="text-md font-black uppercase tracking-[0.3em] text-zinc-500 leading-none">
        {item.label}
      </p>
      <p
    ...
    <div
      className={`p-2 rounded-3xl transition-all duration-500 group-hover:scale-110 shadow-inner
