...
const HUDHeader: React.FC<HUDHeaderProps> = ({ activeView, setActiveView, onMenuClick }) => {
  ...
  return (
    <header className={`h-16 sm:h-20 border-b flex items-center justify-between px-4 sm:px-10 z-60] backdrop-blur-2xl transition-all duration-500 sticky top-0
      ${theme === 'dark' 
        ? 'bg-[#09090b]/90 border-zinc-800/60 shadow-[0_4px_30px_rgba(0,0,0,0.3)]' 
        : 'bg-white/95 border-zinc-200 shadow-[0_4px_20px_rgba(0,0,0,0.05)]'}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col text-left">
            ...
            <span className="text-[8px] font-bold uppercase tracking-widest text-zinc-500 mt-1 hidden sm:block">
              ID: {user?.id?.slice(0, 8).toUpperCase() || 'LOCAL'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 sm:gap-8">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {/* DROP-DOWN MENU */}
          {showDropdown && (
            <div className={`absolute top-full right-0 mt-4 w-64 sm:w-72 rounded-[2.5rem] border shadow-2xl p-3 z-100] animate-in fade-in zoom-in-95 duration-200
              ...
              <div className="px-6 py-5 mb-3 border-b border-zinc-800/40 text-left">
                <p className="text-[10px] font-black uppercase text-zinc-500 tracking-widest mb-1 leading-none">Identity Node</p>
              ...
              <div className="space-y-1">
                <button 
                  ...
                  className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all
                    ${theme === 'dark' ? 'text-zinc-400 hover:bg-zinc-800 hover:text-white' : 'text-zinc-600 hover:bg-zinc-50 hover:text-black'}`}
                >
