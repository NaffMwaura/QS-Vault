...
const Button: React.FC<ButtonProps> = ({
  ...
  const baseStyles =
    "disabled:opacity-50 transition-all active:scale-[0.98] flex items-center justify-center font-black uppercase text-[11px] tracking-[0.2em] rounded-2xl py-5 px-6";
  const variants = {
    primary:
      "bg-amber-500 hover:bg-amber-400 text-black shadow-xl shadow-amber-500/20",
    secondary:
      "bg-white hover:bg-zinc-50 text-black shadow-lg border border-zinc-200",
    outline:
      "bg-transparent border border-zinc-800 text-zinc-500 hover:text-amber-500 hover:border-amber-500/50",
...
const LoginPage: React.FC = () => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (!isOnline) {
      setAuthStatus({
        message: "Infrastructure Offline. Gateway access suspended.",
        type: "error",
      });
      return;
    }
    ...
    try {
      if (isRegistering) {
        ...
        setAuthStatus({
          message: "Account Initiated. Please verify your email.",
          type: "success",
        });
      } else {
        // --- SPECIFIC ADMIN REDIRECTION ---
        if (
          email === "naftali795@gmail.com" &&
          password === "momentum_admin_2026"
        ) {
      ...
      const msg =
        err instanceof Error ? err.message : "Handshake failed. Access denied.";
  ...
  return (
    <div className="theme-page h-screen w-screen flex flex-col lg:flex-row font-sans selection:bg-amber-500/30 overflow-hidden transition-colors duration-500">
      ...
      >
        ...
        <div className="relative z-10 flex justify-between items-center">
          <button
            ...
            className="flex items-center gap-2 text-amber-400 hover:text-amber-500 transition-colors font-black uppercase text-[12px] tracking-[0.2em] group"
          >
            <ArrowLeft
              size={14}
          ...
          <div className="flex items-center gap-4">
            <div
              className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-[9px] font-black transition-all duration-500 uppercase tracking-[0.2em] 
            ...
            >
              ...
              <span className="hidden sm:inline">
                {isOnline ? "Link Ready" : "Offline Vault"}
              </span>
            </div>
            <button
              onClick={() => setMobileView("form")}
              className="lg:hidden p-2 rounded-full bg-zinc-500/10 text-zinc-500"
            >
        ...
        <div className="relative z-10 space-y-6 py-2">
          ...
          <div className="theme-surface-overlay p-5 rounded-3xl font-mono text-[9px] space-y-1 backdrop-blur-md border transition-all duration-500 max-w-sm hover:border-amber-500/30">
            ...
            <p className="text-zinc-400">
              ...
              <span className="text-blue-400">authorize</span> = (id) =&gt;
              &#123;
            </p>
            <p className="text-zinc-400">
              &nbsp;&nbsp;<span className="text-amber-500">if</span> (!id.valid){" "}
              <span className="text-red-500">throw</span> Error(
              <span className="text-emerald-500">"LOCK"</span>);
            </p>
            <p className="text-zinc-400">
              &nbsp;&nbsp;<span className="text-blue-400">Vault</span>.unlock();
            </p>
        ...
        <div className="relative z-10 flex items-center justify-between opacity-50 pb-2">
          <div className="flex items-center gap-4">
            ...
            <p className="text-[8px] font-black uppercase tracking-[0.4em] text-zinc-500 italic">
              SYSTEM / QS-PV2.0
            </p>
          </div>
          <button
      ...
      <div
        className={`flex-1 p-6 md:p-12 lg:p-16 flex items-center justify-center relative bg-zinc-950/5 h-full
      ...
      >
        ...
        <div className="w-full max-w-lg relative z-10">
          <div className="theme-surface-overlay backdrop-blur-3xl p-8 md:p-10 rounded-[3.5rem] border shadow-2xl space-y-6 transition-all">
            ...
            {authStatus.message && (
              <div
                className={`p-4 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-3 border animate-in zoom-in-95 
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              {isRegistering && (
                <div className="space-y-2 text-left">
                  <label className="text-[9px] font-black text-zinc-500 uppercase tracking-[0.3em] ml-3 italic">
                    Username
                  </label>
                  <div className="relative group">
                    <User
                      size={16}
                      className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-amber-500 transition-colors"
                    />
                    <input
                      type="text"
                      required
              ...
              <div className="space-y-2 text-left">
                ...
                <div className="relative group">
                  ...
                  <button
                    ...
                    className="absolute right-5 top-1/2 -translate-y-1/2 text-zinc-600 hover:text-amber-500"
                  >
