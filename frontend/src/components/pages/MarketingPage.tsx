...
const FeatureCard = ({ icon: Icon, title, description, theme }: CardProps) => (
  <GlassCard interactive className="p-10 sm:p-14 border text-center flex flex-col items-center h-full">
    ...
    <h3 className={`text-xl sm:text-2xl font-black uppercase tracking-tight mb-6 ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>
      {title}
    </h3>
    <p className={`text-base leading-relaxed font-medium ${theme === 'dark' ? 'text-zinc-400' : 'text-zinc-500'}`}>
      {description}
    </p>
  </GlassCard>
);
...
const ValuePropCard = ({ icon: Icon, title, description, theme }: CardProps) => (
  <div className={`p-8 sm:p-10 border rounded-[2.5rem] transition-all duration-500 hover:border-amber-500/30 space-y-6 flex flex-col items-start text-left group
    ${theme === 'dark' ? 'bg-zinc-900/60 border-zinc-800' : 'bg-zinc-50 border-zinc-200 shadow-sm'}`}>
    ...
    <div>
      <h3 className={`text-[12px] font-black uppercase tracking-[0.2em] mb-3 ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>
        {title}
      </h3>
      <p className={`text-xs sm:text-sm leading-relaxed font-bold ${theme === 'dark' ? 'text-zinc-400' : 'text-zinc-500'}`}>
        {description}
      </p>
    </div>
  </div>
...
const MarketingPage = ({ onGetStarted }: { onGetStarted: () => void }) => {
  ...
  return (
    <main className={`min-h-screen selection:bg-amber-500/30 transition-colors duration-500 overflow-x-hidden custom-scrollbar
      ${theme === 'dark' ? 'bg-[#09090b] text-white' : 'bg-zinc-100 text-zinc-900'}`}>
      
      {/* 1. PROFESSIONAL NAVIGATION BAR */}
      <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-500
        ${scrolled 
          ? (theme === 'dark' ? "bg-[#09090b]/90 border-zinc-800/50 shadow-2xl" : "bg-white/90 border-zinc-200/50 shadow-lg") + " border-b py-4 backdrop-blur-md" 
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-10 flex justify-between items-center">
          <div className="flex items-center gap-4 sm:gap-10">
            <div className="flex items-center gap-3">
              ...
              <span className="text-2xl font-black uppercase tracking-tighter italic text-amber-600 dark:text-white">
          ...
          <div className="flex items-center gap-4">
            <button onClick={toggleTheme} className="p-3 text-zinc-500 hover:text-amber-500 transition-all active:scale-90">
            ...
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="lg:hidden p-2 text-zinc-500">
      ...
      {/* MOBILE MENU */}
      {mobileMenuOpen && (
        <div className={`fixed inset-0 z-45] lg:hidden backdrop-blur-2xl transition-all duration-300 ${theme === 'dark' ? 'bg-[#09090b]/95' : 'bg-white/95'}`}>
          <div className="flex flex-col items-center justify-center h-full space-y-8">
             ...
             <button onClick={() => navigate('/login')} className={`text-2xl font-black uppercase tracking-widest ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>Login</button>
          </div>
        </div>
      )}
      ...
      <header className="relative min-h-screen flex items-center justify-center px-6 pt-32 overflow-hidden bg-transparent">
        <div className="absolute inset-0 z-0 pointer-events-none">
           <div className={`absolute top-0 right-0 w-2/3 h-2/3 rounded-full blur-[160px] -translate-y-1/2 translate-x-1/4 ${theme === 'dark' ? 'bg-amber-500/10' : 'bg-amber-500/5'}`} />
           <div className={`absolute bottom-0 left-0 w-1/2 h-1/2 rounded-full blur-[140px] translate-y-1/4 -translate-x-1/4 ${theme === 'dark' ? 'bg-zinc-500/10' : 'bg-zinc-500/5'}`} />
        </div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto text-center px-6">
          ...
          <h1 className="text-4xl sm:text-6xl md:text-7xl font-black mb-12 leading-[1.1] tracking-tighter transition-colors">
            <span className={`block min-h-[1.1em] text-transparent bg-clip-text transition-all duration-1000 ${theme === 'dark' ? 'bg-linear-to-b from-white to-zinc-600' : 'bg-linear-to-b from-zinc-900 to-zinc-500'}`}>
          ...
          <p className="text-lg sm:text-2xl md:text-3xl mb-24 max-w-5xl mx-auto font-medium leading-relaxed px-6 text-zinc-500 dark:text-zinc-400">
      ...
      {/* 3. WORKFLOW SECTION */}
      <section className={`py-60 px-6 sm:px-12 border-y relative overflow-hidden transition-colors duration-500
        ${theme === 'dark' ? 'bg-zinc-900/50 border-zinc-800' : 'bg-zinc-50 border-zinc-200'}`}>
        <div className="max-w-7xl mx-auto text-center relative z-10">
          <h2 className={`text-5xl sm:text-8xl font-black mb-48 tracking-tighter uppercase leading-none ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-16 mb-48 text-left">
            <div className="max-w-4xl">
              <h2 className={`text-4xl sm:text-7xl font-black tracking-tight mb-10 uppercase leading-[1.05] ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>
                Engineering-Grade Precision,<br className="hidden lg:block" /> Built for the Site Node.
              </h2>
              <p className={`text-xl sm:text-2xl font-medium ${theme === 'dark' ? 'text-zinc-400' : 'text-zinc-500'}`}>
            ...
            <div className={`w-full lg:w-auto border p-12 rounded-[2.5rem] shadow-2xl transition-colors
              ...
              <p className={`text-4xl font-black italic leading-none ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>SMM-KE / RICS 2026</p>
            </div>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-14">
            <ValuePropCard theme={theme} icon={Zap} title="Offline-First Logic" description="Your data stays safe locally on your laptop and syncs to the cloud the second you have a signal." />
            <ValuePropCard theme={theme} icon={MapPin} title="Localized SMM Rules" description="Fully aligned with East African SMM for roads and civil works. No more manual compliance checking." />
            <ValuePropCard theme={theme} icon={TrendingUp} title="Project Analytics" description="Advanced algorithms track material consumption and workforce productivity in real-time." />
            <ValuePropCard theme={theme} icon={LayoutGrid} title="Digital Diary" description="Keep an immutable record of site events, weather, and deliveries linked directly to your cost nodes." />
            <ValuePropCard theme={theme} icon={CheckCircle} title="Professional Reports" description="Export valuation reports, BoQs, and Variation claims in standard PDF and Excel formats." />
            <ValuePropCard theme={theme} icon={ShieldCheck} title="Verified Security" description="AES-256 local encryption and secure cloud backups ensure your project data is never compromised." />
          </div>
        </div>
      </section>

      {/* 5. TRUSTED NODES */}
      <section className={`py-64 px-6 sm:px-12 overflow-hidden relative transition-colors duration-500 ${theme === 'dark' ? 'bg-[#09090b]' : 'bg-zinc-100'}`}>
        <div className="max-w-7xl mx-auto text-center relative z-10">
          <h2 className={`text-4xl sm:text-8xl font-black uppercase tracking-tighter mb-48 italic leading-none ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>
          ...
          <div className="grid md:grid-cols-2 gap-24 text-left">
            <GlassCard className="p-16 border shadow-2xl flex flex-col justify-between h-full">
              <p className={`font-medium italic mb-12 leading-relaxed text-xl sm:text-2xl ${theme === 'dark' ? 'text-zinc-400' : 'text-zinc-600'}`}>
              ...
              <div className="flex items-center gap-6 mt-auto">
                ...
                <div>
                  <p className={`font-black text-sm uppercase tracking-[0.2em] ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>Denzel Damba</p>
            ...
            <GlassCard className="p-16 border shadow-2xl flex flex-col justify-between h-full">
              ...
              <div className="flex items-center gap-6 mt-auto">
                <div className="w-16 h-16 rounded-3xl bg-zinc-800 border border-zinc-700 flex items-center justify-center font-black text-amber-500 text-2xl shadow-xl shrink-0 italic">NM</div>
                <div>
                  <p className={`font-black text-sm uppercase tracking-[0.25em] ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>Naff Mwaura</p>
      ...
      {/* 6. SYSTEM FOOTER */}
      <footer className={`py-40 text-center border-t transition-colors duration-500 ${theme === 'dark' ? 'bg-[#09090b] border-zinc-800' : 'bg-zinc-50 border-zinc-200'}`}>
        <div className="mb-14 flex items-center justify-center gap-4">
            ...
            <span className={`text-4xl sm:text-5xl font-black uppercase tracking-tighter italic ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>
              QS VAULT<span className="text-amber-500">.</span>
            </span>
        </div>
